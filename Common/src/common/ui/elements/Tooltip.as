...
public class Tooltip extends Sprite {
    ...
    private function getXPos():Number {
        ...
        if (mouseX < Global.StageWidth / 2) { // Center orientation: right
            if (mouseX < 0) { // Left limit
                return 0;
            }
            if (mouseX + limitWidth > Global.StageWidth) { // When tooltip touches screen right
                return Global.StageWidth - limitWidth;
            }
            return mouseX;
        }

        if (mouseX >= Global.StageWidth / 2) { // Center orientation: left
            if (mouseX > Global.StageWidth) { // Right limit
                return Global.StageWidth - limitWidth;
            }
    ...
    private function getYPos():Number {
        ...
        if (mouseY < Global.StageHeight / 2) { // Center orientation: up
            if (mouseY < 0) { // Top limit
                return 0;
            }
            if (mouseY + limitHeight > Global.StageHeight) { // When tooltip touches screen bottom
                return Global.StageHeight - limitHeight;
            }
            return mouseY;
        }

        if (mouseY >= Global.StageHeight / 2) { // Center orientation: left
            if (mouseY > Global.StageHeight) { // Right limit
                return Global.StageHeight - limitHeight;
            }
